// gateway.js — API Gateway Experimento 2 (Security ASR)
// Basado en el código entregado; única adición: /test/reset-limiter y trust proxy
...
const app = express();
app.set('trust proxy', true); // permite leer X-Forwarded-For como req.ip
...
// Solo para pruebas: resetea el contador del rate limiter para la IP actual
app.post('/test/reset-limiter', (req, res) => {
  limiter.resetKey(req.ip);
  res.json({ reset: true, ip: req.ip });
});
...
app.get('/dashboard/summary', verifyToken, async (req, res) => {
  ...
  try {
    ...
    res.json({ inventory, reports, elapsed_ms: Date.now() - start, cached: false });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

app.listen(8000, () => console.log('[EXP2] API Gateway corriendo en :8000'));
